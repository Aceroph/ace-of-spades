...
class School(subclasses.Cog):
    ...
    async def schedule(self, ctx: commands.Context, date: misc.Time(return_type='date')=datetime.date.today()): # type: ignore
        ...
        # errors
        if date < self.start or date > self.stop or date in self.noschool or date.weekday() in [5, 6]:
            return await ctx.reply(f":warning: You don't even have school on {date.strftime('%Y-%m-%d')}")
        
        # embed
        embed = discord.Embed(color=discord.Color.green(), title="School Schedule", description=f"{misc.curve} {date} (Day {day})")
        for i, subject in enumerate(schedule):
            ...
            embed.add_field(name=f"[ {subject} ] {data['descriptionShort']}", value=f"{misc.space}:teacher: {teacher['firstName']} {teacher['lastName']}\n{misc.space}\N{BUSTS IN SILHOUETTE} Group {data['group']}\n{misc.space}\N{CLOCK FACE TEN-THIRTY} {self.hours[i]}\n", inline=False)
