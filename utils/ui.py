...
class ModuleMenu(subclasses.View):
    def __init__(self, bot: "AceBot"):
        ...
        self.bot = bot
        options = [
            discord.SelectOption(label=name, value=name, emoji=module.emoji)
    ...
    class Embed(discord.Embed):
        def __init__(self, bot: "AceBot"):
            ...
            self.set_thumbnail(
                url="https://static-00.iconduck.com/assets.00/cog-settings-icon-2048x1619-0lz5tnft.png"
            )
            self.count = 0
        ...
        def list_modules(self, bot: "AceBot", selected: subclasses.Cog = None):
            data = {
                ...
                "value": "\n".join(
                    [
                        f"{bot.get_cog(name).emoji} {f'`{name}`' if selected and selected.qualified_name == name else name}"
        ...
        def module_info(self, bot: "AceBot", cog: subclasses.Cog = None):
            if cog:
                ...
                data = {
                    "name": cog.qualified_name + " Module",
                    "value": f"Loaded\n⤷ <t:{int(cog.time)}:R>\n\nStats\n⤷ Commands used : {cog.cmds}\n⤷ Lines : {self.count}\n⤷ [Source]({misc.git_source(bot, cog)})",
                    "inline": True,
                }
                return (
                    self.set_field_at(1, **data)
                    if len(self.fields) > 1
                    else self.insert_field_at(1, **data)
                )
    ...
    class Select(discord.ui.Select["ModuleMenu"]):
        ...
        async def callback(self, interaction: discord.Interaction):
            ...
            embed = self.view.Embed.from_embed(
                self.view.bot, interaction.message.embeds[0]
            )
            ...
            if module is not None:
                ...
                try:
                    ...
                    await interaction.response.send_message(
                        f":repeat: Reloaded {module} `(Took around {round(time.time()-start, 4)}s)`",
                        ephemeral=True,
                    )
                except Exception as err:
                    await interaction.response.send_message(
                        f":warning: An error occured while trying to reload {module} ! `{err}`",
                        ephemeral=True,
                    )
            else:
                await interaction.response.send_message(
                    ":warning: Did not select any module !", ephemeral=True
                )
        else:
